...
clay_radio_button :: proc(
    base_id_string: string,
    current_variant: $E,
    names: [E]string,
    disabled: bool = false, 
) -> (result: E, updated: bool) where intrinsics.type_is_enum(E) {
    if clay.UI()({
        layout = {
            layoutDirection = .TopToBottom,
        },
    }) {
        for variant, index in E {
            if clay.UI()({
                layout = {
                    childAlignment = {
                        y = .Center,
                ...
                if clay_button(clay.ID(base_id_string, u32(index)), 
                    sizing = clay.Sizing {
                        clay.SizingFixed(f32(scaled_info_font_size - 2 * scaled_border)),
                        clay.SizingFixed(f32(scaled_info_font_size - 2 * scaled_border)),
                    },
                    icon = .Dot if variant == current_variant  else nil,
                    disabled = disabled,
                ) {
                    result = variant
                    updated = true
                ...
                clay.TextDynamic(names[variant], clay.TextConfig({
                    fontId = FONT_PALETTE[.Default_Regular],
                    fontSize = scaled_info_font_size,
                    textColor = PALETTE[.Mid_Gray] if disabled else PALETTE[.Light_Gray],
                }))
            }
        }
    }
...
clay_layout :: proc(gs: ^Game_State) -> Render_Command_Array {
    ...
    if clay.UI(id = clay.ID("outer_container"))({
        ...
        case .Title:    clay_title_screen(gs)
        case .Lobby:    clay_lobby_screen(gs)
        case .Game:     clay_game_screen(gs)
        }
    }
